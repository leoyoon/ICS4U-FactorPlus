...
class GraphingPoints{ //Start of class GraphingPoints
    ...
    init() {
        ...
        for (x = -5; x<5; x += 1) { //This for loop fills the pointX, pointY and pointYString arrays
            
            y = a*(x-h)*(x-h)+k //This is the basic formula for creating a quadratic
            
            pointX.insert(String(x), at: x+5) //x values (loop number) is stored in the pointX array
            pointYY.insert(Double(y), at: x+5) //y values (from equation above) are stored in the pointY array
            pointYString.insert(String(y), at: x+5) 
        ...
        pointX.insert("-6", at: 0)
        pointX.insert("6", at: 11)
        ...
        pointYY.insert(abs(Double(placeholderMax)*1.2)+Double(abs(a*10)), at: 0)
        pointYY.insert(Double(placeholderMax+(a*5)), at: 11)
    }
