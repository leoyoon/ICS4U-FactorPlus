...
class MultipleChoiceTimerViewController: UIViewController {
    ...
    var timer = Timer()               //the 30 seconds timer
    var repeatTimer = Timer()         //repeats every second to update the timerLabel
    ...
    override func viewDidLoad()
    {
        ...
        let temp = Double(numQuestions)/10
        ...
        coverUpButton.isHidden = true                             //hide the disabler to begin
        ...
        if (fromPause == true) {
            ...
            timer = Timer.scheduledTimer(timeInterval: Double(timeRemaining), target: self, selector: #selector(MultipleChoiceTimerViewController.thirtySeconds(_:)), userInfo: nil, repeats: false)
            
            //re-initialize the repeatTimer
            repeatTimer = Timer.scheduledTimer(timeInterval: 1.0, target: self, selector: #selector(MultipleChoiceTimerViewController.secondPassed(_:)), userInfo: nil, repeats: true)
            ...
            timer = Timer.scheduledTimer(timeInterval: 30.0, target: self, selector: #selector(MultipleChoiceTimerViewController.thirtySeconds(_:)), userInfo: nil, repeats: false)
    ...
    func makeQuestion() {
        ...
        let quadraticRelation = quadratic(numCorrect: numCorrect) //the quadratic object, which generates a quadratic relation
    ...
    func makeMultipleChoice() {
        ...
        for ( a = 0; a <= 3; a += 1) {
            choice.insert("", at: a)   //fill in the array with blanks
        } //end of forloop
        ...
        choice.remove(at: 0) //remove the value at first index,
        ...
        if (basicFactor == true) {
            ...
            choice.insert("(x"+rS+")(x"+sS+")", at: 0) //and then fill it with the correct one
        }
        else {
            ...
            choice.insert("("+kS+"x"+rS+")("+jS+"x"+sS+")", at: 0) //and then fill it with the correct one
        }
        finalAns = choice[0]
        
        for (i = 1; i <= 3; i += 1) //for 3 times - which is excluding the 0th index
        {
                ...
                */
                for (checkAnyAns: Int in 0 ..< i) {
                    ...
                    if (choice[i] == choice[checkAnyAns]){
                        ...
                        choice.remove(at: i)
                        choice.insert("", at: i)
                        i -= 1 //i would equal the same thing once the forloop ends, since
    ...
    func selectionSort() {
        ...
        let rightAns: String = choice [0] //the right answer is stored in the first index thanks to
        //the makeMultipleChoice() func
        choice.remove(at: 0)           //remove the value for now
        
        rightAnsIndex = 4 - Int(arc4random_uniform (4) + 1) //pick a random index to store the correct answer in
        choice.insert(rightAns, at: rightAnsIndex)     //then insert the value in the array
    ...
    func buttonClicked(_ buttonIndex: Int) {
        ...
        if (checkForRightAnswer(buttonIndex) == true) {
            ...
            numCorrect += 1                                                          //increment number of answers correctly chosen
        }
        ...
        buttonFrame.isHidden = false       //show the buttonFrame
    ...
    func checkForRightAnswer(_ buttonNumber: Int) -> Bool
    {
    ...
    @IBAction func nextButtonClicked(_ sender: AnyObject) {
        ...
        pauseButton.isHidden = false  //show the pause button
        nextButton.isHidden = true    //hide the next button
        buttonFrame.isHidden = true  //hide the buttonFrame
        coverUpButton.isHidden = true //hide the disabler
        pauseImage.isHidden = false   //show the pause image
    ...
    func changeProgress() {
        ...
        numQuestions += 1                                        //increment the number of questions answered
        let temp = Double(numQuestions)/10                    //the temp variable to be used for the progress bar
    ...
    func endGame() {
        ...
        performSegue(withIdentifier: "endMCFT", sender: self) //proceed to the EndViewController
    ...
    @IBAction func pauseClicked(_ sender: AnyObject) {
        ...
        performSegue(withIdentifier: "pauseMCFT", sender: sender) //proceed to the PauseViewController
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any!) {
        ...
        if(segue.identifier == "pauseMCFT") {
            ...
            let pvc = segue.destination as! PauseViewController //call the PauseViewController
            ...
            let evc = segue.destination as! EndViewController //call the EndViewController
    ...
    func thirtySeconds(_ timer: Timer) {
        ...
        buttonFrame.isHidden = false
        pauseButton.isHidden = true
        coverUpButton.isHidden = false
    ...
    //do this each second
    func secondPassed(_ repeatTimer: Timer) {
