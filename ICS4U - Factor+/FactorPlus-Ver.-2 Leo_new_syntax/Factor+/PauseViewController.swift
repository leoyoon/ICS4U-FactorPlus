...
class PauseViewController: UIViewController {
    ...
    @IBAction func restartGame(_ sender: AnyObject) { //if restart button pressed, score and question number is set to zero, and restarting is set to true
        score = 0
        numQuestion = 0
        restarting = true
        continueToGame()
    }
    @IBAction func continueGame(_ sender: AnyObject) { //when continue button pressed, no values are set to zero and proceeds to continueToGame()
        
        continueToGame()
    }
    @IBAction func goToMenu(_ sender: AnyObject) { //when main menu button pressed, score, number of question, and type is set to zero or "" and proceeds stright to perform segue method
        score = 0
        numQuestion = 0
        type = ""
    ...
    func continueToGame (){ //according to type, the program proceeds to perform segue with the corresponding identifier
        ...
        {
            performSegue(withIdentifier: "Continue MCF", sender: self)
        }
        else if(type == "Multiple Choice Factor Timer")
        {
            performSegue(withIdentifier: "Continue MCFT", sender: self)
        }
        else if(type == "User Input Factor")
        {
            performSegue(withIdentifier: "Continue UIF", sender: self)
        }
        else if(type == "Multiple Choice Graph")
        {
            performSegue(withIdentifier: "Continue MCG", sender: self)
        }
        else if(type == "User Input Graph")
        {
            performSegue(withIdentifier: "Continue UIG", sender: self)
        }
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) { //method that takes in an identifier and proceeds to do the corresponding segue
        ...
        //if the identifier is a gamemode, it will load up the corresponding gamemode screen
            ...
            let mcvc = segue.destination as! MultipleChoiceViewController
            ...
            for (i in 0 ..< 4){
                mcvc.choice.insert(multipleChoiceChoices[i], at: i)
            }
        }
        else if(segue.identifier == "Continue MCFT")
        {
            let mcftvc = segue.destination as! MultipleChoiceTimerViewController
            ...
            for (i in 0 ..< 4){
                mcftvc.choice.insert(multipleChoiceChoices[i], at: i)
            }
        }

        else if(segue.identifier == "Continue UIF")
        {
            let uifvc = segue.destination as! UIFViewController
        ...
        {
            let mc2vc = segue.destination as! MultipleChoice2ViewController
            ...
            if (restarting == false) {
                ...
                for (i in 0 ..< 4){
                    mc2vc.choice.insert(multipleChoiceChoices[i], at: i)
                }
                
                for (i in 0 ..< 12) {
                    mc2vc.xValues.insert(xval[i], at: i)
                    mc2vc.yValues.insert(yval[i], at: i)
                }
        ...
        {
            let uigvc = segue.destination as! UIGViewController
            ...
            if (restarting == false) {
                ...
                for (i in 0 ..< 12) {
                    uigvc.xValues.insert(xval[i], at: i)
                    uigvc.yValues.insert(yval[i], at: i)
                }
                
                uigvc.graphPoint.insert(graphPoint[0], at: 0)
            }
